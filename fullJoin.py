...
In the reducer, it reads in dictionary of all the offers.  It then reads in the 
values for all the transactions and the history.  It then combines the history 
data with the offer and joins that with each element in the transactions for the
user, after checking whether the transaction company/price/product is in the 
offer set for the user.  
'''
...
class MRWordFrequencyCount(MRJob):
    ...
    def reducer(self, key, values):
        #Offer: offer, category, quantity, company, offervalue, brand
        ...
        if len(trans) > 0 and len(history) > 0:
            for t in trans:
                #If offer lenght is greater than zero, compare to see if transaction is relevant
                if len(offer) > 0:
                    if(offer[1] == t[2] or offer[3] == t[3] or offer[5] == t[4]):
